...
constexpr char _filter[] PROGMEM = R"(
[
  {
 ...
 * API request handler
 */
boolean requestJsonApi(JsonDocument& doc, JsonDocument& filter, String url, String payload = "", size_t capacity = 0, String type = "POST", boolean sendAuth = false) {
  // WiFiClient
...
void handleStartDevicelogin() {
  ...
  if (state != SMODEDEVICELOGINSTARTED) {
    ...
    boolean res = requestJsonApi(doc, filter, "https://login.microsoftonline.com/" + String(paramTenantValue) + "/oauth2/v2.0/devicecode", "client_id=" + String(paramClientIdValue) + "&scope=offline_access%20openid%20Presence.Read", capacity);
